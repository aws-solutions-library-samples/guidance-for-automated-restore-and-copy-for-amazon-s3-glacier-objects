...
report_prefix = str(os.environ['batch_ops_redrive_report_prefix'])
...
def lambda_handler(event, context):
    ...
    manifest_s3Bucket = str(event['Records'][0]['s3']['bucket']['name'])
    manifest_s3Key = parse.unquote_plus(event['Records'][0]['s3']['object']['key'], encoding='utf-8')
    logger.info(manifest_s3Bucket)
    logger.info(manifest_s3Key)

    # Specify Manifest Fields #
    manifest_fields_count = None
    if "redrive-failed-copy/csv-manifest/with-version-id/" in manifest_s3Key:
        logger.info("Use Version ID in manifest")
        manifest_fields_count = str(3)
    elif "redrive-failed-copy/csv-manifest/no-version-id/" in manifest_s3Key:
        logger.info("Don't use Version ID in Manifest")
        manifest_fields_count = str(2)
    elif "redrive-failed-copy/eventbridge/with-version-id/" in manifest_s3Key:
        logger.info("Use Version ID in Manifest")
        manifest_fields_count = str(3)
    elif "redrive-failed-copy/eventbridge/no-version-id/" in manifest_s3Key:
        logger.info("Don't use Version ID in Manifest")
        manifest_fields_count = str(2)                                    



    copy_job_id = s3_batch_ops_copy(manifest_s3Bucket, manifest_s3Key, manifest_fields_count)
    logger.info(copy_job_id)

    my_sns_message = f'Copy Job {copy_job_id} Successfully Submitted to Amazon S3 Batch Operation'
...
def s3_batch_ops_copy(manifest_bucket, manifest_key, manifest_flds_num):
    manifest_fields = []
    if "redrive-failed-copy/csv-manifest/" in manifest_key:
    ...
    elif "redrive-failed-copy/eventbridge/" in manifest_key:   
        if manifest_flds_num == '3':
    ...
    # Set Description #
    my_job_description = f"Redrive Failed Copy Tasks by AutoRestoreCopy Solution for S3Bucket: {my_s3_bucket}"    
            ...
            Tags=[
                {
                    ...
                    'Value': 'Redrive for Failed Tasks'
                },
            ]
        )
